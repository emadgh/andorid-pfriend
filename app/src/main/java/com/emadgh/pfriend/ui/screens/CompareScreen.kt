...
fun CompareScreen(
    ...
    circleId: Long?,
    onBack: (() -> Unit)? = null,
    onUser: (ComparisonRow) -> Unit
) {
    Column(Modifier.fillMaxSize()) {
        val headerMotion = if (circleId != null) {
            Modifier.materialSharedBounds(MotionKeys.circleCompare(circleId))
        } else {
            Modifier
        }
        ...
        Surface(
            ...
            tonalElevation = if (circleId != null) 1.dp else 0.dp
        ) {
            Row(
                ...
                Column {
                    ...
                    Text(
                        if (circleId != null) "Circle members" else "All registered users",
        ...
        Column(Modifier.fillMaxWidth().horizontalScroll(scroll)) {
            ...
            LazyColumn(Modifier.width(760.dp).fillMaxHeight()) {
                items(rows, key = { it.user.id }) { row ->
                    CompareRowView(row, circleId, onUser)
                }
...
private fun CompareHeader() {
    Surface(tonalElevation = 2.dp) {
        Row(Modifier.width(760.dp).padding(vertical = 12.dp, horizontal = 12.dp)) {
            HeaderCell("Person", 170f)
            HeaderCell("Water", 100f)
            HeaderCell("Food", 100f)
            HeaderCell("Urine", 85f)
            HeaderCell("Urine ml", 100f)
            HeaderCell("Bowel", 85f)
        }
    }
}
...
private fun CompareRowView(
    ...
    TextButton(
        ...
        modifier = Modifier.materialSharedBounds(MotionKeys.compareUser(row.user.id, circleId))
    ) {
        Row(
            Modifier.width(760.dp).padding(vertical = 10.dp, horizontal = 12.dp),
            verticalAlignment = Alignment.CenterVertically
        ) {
            Column(Modifier.width(170.dp)) {
                Text(row.user.displayName, fontWeight = FontWeight.SemiBold)
                Text("@${row.user.username}", style = MaterialTheme.typography.bodySmall)
            }
